...
<style>
    ...
    .card-info.kondisi-A { border-left-color: #198754; }
    .card-info.kondisi-B { border-left-color: #ffc107; }
    .card-info.kondisi-C { border-left-color: #dc3545; }
    .price-display { font-size: 1.8rem; font-weight: bold; color: #198754; }
    .preview-box { background-color: #f8f9fa; border: 2px dashed #dee2e6; border-radius: 8px; padding: 15px; }
    .btn-pos-submit { font-size: 1.2rem; padding: 12px; font-weight: bold; }
    
    /* Layout styling for disabled right column */
    #formKonfirmasi { opacity: 0.5; pointer-events: none; transition: opacity 0.3s; }
    #formKonfirmasi.active { opacity: 1; pointer-events: auto; }
</style>
                    ...
                    <form id="formPenjualan">
                        ...
                        <div class="mb-4">
                            ...
                            <div class="btn-group w-100" role="group">
                                ...
                                <input type="radio" class="btn-check" name="metode_bayar" id="btnQris" value="qris" autocomplete="off">
                                <label class="btn btn-outline-primary" for="btnQris"><i class="bi bi-qr-code-scan"></i> QRIS</label>

                                <input type="radio" class="btn-check" name="metode_bayar" id="btnTransfer" value="transfer" autocomplete="off">
                                <label class="btn btn-outline-info" for="btnTransfer"><i class="bi bi-bank"></i> Transfer</label>
                            </div>
                        </div>
...
document.addEventListener('keydown', (e) => { if(e.key === 'Escape') suggestionBox.style.display = 'none'; });
document.addEventListener('click', (e) => { if(!inputCari.contains(e.target) && !suggestionBox.contains(e.target)) suggestionBox.style.display = 'none'; });
...
function selectItem(item) {
    ...
    document.getElementById('infoFoto').onerror = function() { this.src = 'https://via.placeholder.com/200x200?text=No+Photo'; };
    ...
    if(item.kondisi === 'A') { badge.classList.add('bg-success'); border.classList.add('kondisi-A'); }
    else if(item.kondisi === 'B') { badge.classList.add('bg-warning', 'text-dark'); border.classList.add('kondisi-B'); }
    else { badge.classList.add('bg-danger'); border.classList.add('kondisi-C'); }
