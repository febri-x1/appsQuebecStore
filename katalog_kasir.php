...
$stmtCount = $pdo->prepare("SELECT COUNT(*) FROM barang $whereClause");
$stmtCount->execute($params);
$totalData = $stmtCount->fetchColumn();
$totalPages = ceil($totalData / $limit);

$sql = "SELECT * FROM barang $whereClause ORDER BY id DESC LIMIT $limit OFFSET $offset";
...
<style>
    ...
    .badge-kondisi-A { background-color: #198754; }
    .badge-kondisi-B { background-color: #ffc107; color: #000; }
    .badge-kondisi-C { background-color: #dc3545; }
    .price-tag { font-size: 1.25rem; font-weight: bold; color: #0d6efd; }
</style>
                ...
                <div class="col-md-3">
                    <select name="kategori" class="form-select border-2">
                        ...
                        foreach ($kategori_options as $kat): ?>
                            <option value="<?= $kat ?>" <?= $kategori_filter == $kat ? 'selected' : '' ?>><?= ucfirst(str_replace('_', ' ', $kat)) ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="col-md-7">
